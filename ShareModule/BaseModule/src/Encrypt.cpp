...
void CMD5Encrypt::EncryptData(LPCTSTR pszSrcData, TCHAR szMD5Result[33])
{
	...
	//输出结果
	szMD5Result[0] = 0;
	for (int i = 0; i < 16; i++) wsprintf(&szMD5Result[i * 2], _T("%02x"), szResult[i]);

	return;
}

void CMD5Encrypt::EncryptData_CapitalHex(LPCTSTR pszSrcData, TCHAR szMD5Result[33])
{
	//加密密文
	CMD5 MD5Encrypt;
	UCHAR szResult[16];
...
WORD CXOREncrypt::EncryptData(LPCTSTR pszSrcData, LPTSTR pszEncrypData, WORD wSize)
{
	...
	for (WORD i = 0; i < wTimes; i++)
	{
		...
		wsprintf(pszEncrypData + i * 4, _T("%02x%02x"), szRandKey[i%ENCRYPT_KEY_LEN], bTempCode);
	}
...
WORD CXOREncrypt::CrevasseData(LPCTSTR pszEncrypData, LPTSTR pszSrcData, WORD wSize)
{
	...
	WORD wSoureLength = (WORD)_tcstol(szTempBuffer, &pEnd, 16);
	...
	for (int i = 0; i < wSoureLength; i++)
	{
		...
		UINT uKey = _tcstol((LPCTSTR)szKeyBuffer, &pEnd, 16);
		UINT uEncrypt = _tcstol(szTempBuffer, &pEnd, 16);

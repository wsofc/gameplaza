...
namespace UserMenuNSP
{
	...
	LostCount* LostCount::g_instance = nullptr;
	LostCount::LostCount()
	...
	LostCount* LostCount::GetInstance()
	{
		if (!g_instance)
		{
			g_instance = new LostCount();
		}

		return g_instance;
	}

	int LostCount::CreatFrame(CPaintManagerUI* pParentManager)
	{
	...
	void LostCount::InitWindow()
	{
		// 初始化控件名
		m_strGameID		= _T("panel_lost_count-edt_game_id");
		m_strCount		= _T("panel_lost_count-edt_count");
		m_strCancel		= _T("panel_lost_count-btn_cancel");
		m_strEnsure		= _T("panel_lost_count-btn_ensure");
		m_strClose		= _T("panel_lost_count-btn_close");
	...
	void LostCount::InitData(TNotifyUI& msg)
	{
		// 当前玩家ID
	...
	void LostCount::UpdateLastData()
	{
		// 更新上次设置数据
		_stprintf_s(m_pzBuffer, _T("%lld"), g_tMenuData.lCurLostCount);
		m_pEdtCount->SetText(m_pzBuffer);
	}

	void LostCount::Notify(TNotifyUI& msg)
	{
	...
	void LostCount::Check()
	{
		...
		else if (!MatchRegular(pzCount, _T("[0-9]+")))
		{
			...
			iMsgEventID = MsgEventID::MSGID_LOST_COUNT;
			goto lab_err;
		}
		else
		{
			if (g_pDelegate)
			{
				...
				g_pDelegate->CallBackLostCount(lGameID, lCount);
			}
			this->Close();
		}
	...
	lab_err:
		if (bInputError)
		{
			...
			DialogCallBack dlg_cb = CC_CALLBACK_2(LostCount::MsgBoxProcFocus, this);
	...
	void LostCount::MsgBoxProcFocus(DLG_ID id, TNotifyUI& msg)
	{
		switch (id)
		{
			case MsgEventID::MSGID_USER_GAMEID:	m_pEdtGameID->SetFocus();	break;
			case MsgEventID::MSGID_LOST_COUNT:	m_pEdtCount->SetFocus();	break;
			default:														break;
		}
	}

	LRESULT LostCount::HandleCustomMessage(UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL& bHandled)
	{
		switch (uMsg)
		{
			case WM_CHAR:
	...
	void LostCount::OnFinalMessage(HWND hWnd)
	{
	...
	UILIB_RESOURCETYPE LostCount::GetResourceType() const
	{
		return g_tMenuData.tResInfo.iResourceType;
	}

	LPCTSTR LostCount::GetResourceID() const
	{
		return g_tMenuData.tResInfo.iResourceID;
	}

	CDuiString LostCount::GetSkinFolder()
	{
		return _T("");
	}

	CDuiString LostCount::GetSkinFile()
	{
		return IDR_XML_PanelLostCount;
	}

	LRESULT LostCount::ResponseDefaultKeyEvent(WPARAM wParam)
	{

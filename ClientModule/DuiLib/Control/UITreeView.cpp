...
namespace DuiLib
{
	...
	void CTreeNodeUI::PaintSepImage(HDC hDC)
	{
		int iSepWidth = 1;
		RECT rc = { m_rcItem.left, m_rcItem.bottom - iSepWidth, m_rcItem.right, m_rcItem.bottom };
		if (!m_sSepImage.IsEmpty())
		{
			bool bRes = CRenderEngine::DrawImageString(hDC, m_pManager, rc, m_rcPaint, m_sSepImage, _T("corner='2,0,2,0'"));
			if (!bRes) m_sSepImage.Empty();
		}
	}

	void CTreeNodeUI::PainrSetColor(HDC hDC)
	{
		int iSepWidth = 1;
		RECT rc = { m_rcItem.left, m_rcItem.bottom - iSepWidth, m_rcItem.right, m_rcItem.bottom };
		if (m_dwSepColor != 0) CRenderEngine::DrawColor(hDC, rc, m_dwSepColor);
	}

	void CTreeNodeUI::DoPaint(HDC hDC, const RECT& rcPaint)
	{
	...
	void CTreeNodeUI::SetSepColor(DWORD dwColor)
	{
	...
	DWORD CTreeNodeUI::GetSepColor()
	{
		return m_dwSepColor;
	}

	void CTreeNodeUI::SetSepImage(LPCTSTR pStrImage)
	{
		if (_tcscmp(m_sSepImage, pStrImage) == 0) return;

		m_sSepImage = pStrImage;
		Invalidate();
	}

	LPCTSTR CTreeNodeUI::GetSepImage()
	{
		return m_sSepImage;
	}
	...
	bool CTreeViewUI::Add(CTreeNodeUI* pControl)
	{
		...
		pControl->SetSepImage(m_sSepImage);
		pControl->SetSepColor(m_dwSepColor);
	...
	bool CTreeViewUI::OnCheckBoxChanged(void* param)
	{
		...
		if (_tcscmp(pMsg->sType, DUI_MSGTYPE_SELECTCHANGED) == 0)
		{
	...
	{
		TNotifyUI* pMsg = (TNotifyUI*)param;
		if (_tcscmp(pMsg->sType, DUI_MSGTYPE_ITEMDBCLICK) == 0)
		{
			CTreeNodeUI* pItem = static_cast<CTreeNodeUI*>(pMsg->pSender);
			CCheckBoxUI* pFolder = pItem->GetFolderButton();

			//edit bt Aries 2016.6.21 当折叠按钮"接收鼠标消息"开关打开时，消息才能继续传传递
			if (pFolder->IsMouseEnabled())
			{
	...
	// 折叠根节点时,也顺带折叠对应子节点(展开时之展开一级子节点,折叠时折叠所有子节点)  Modify by Aries 2017/11/15
	void CTreeViewUI::SetItemExpand(bool _Expanded, CTreeNodeUI* _TreeNode /*= NULL*/)
	{
		if (_TreeNode)
		{
			if (_TreeNode->GetCountChild() > 0)
			{
				...
				for (int nIndex = 0; nIndex < nCount; nIndex++)
				{
					...
					CCheckBoxUI* pFolderButton = pItem->GetFolderButton();
					if (!pFolderButton) continue;

					if (pItem->GetCountChild() > 0 && pFolderButton->IsSelected())
					{
						pFolderButton->Selected(_Expanded);
		...
		else
		{
			...
			{
				CTreeNodeUI* pItem = (CTreeNodeUI*)GetItemAt(i);
				if (pItem && pItem->GetCountChild() > 0)
				{
					pItem->SetVisible(_Expanded);
					CCheckBoxUI* pFolderButton = pItem->GetFolderButton();
					if (!pFolderButton) continue;
	...
	void CTreeViewUI::SetSepColor(DWORD dwColor)
	{
		if (m_dwSepColor == dwColor)return;
	...
	DWORD CTreeViewUI::GetSepColor()
	{
		return m_dwSepColor;
	}

	void CTreeViewUI::SetSepImage(LPCTSTR pStrImage)
	{
		m_sSepImage = pStrImage;
	}

	LPCTSTR CTreeViewUI::GetSepImage()
	{
		return m_sSepImage;
	}

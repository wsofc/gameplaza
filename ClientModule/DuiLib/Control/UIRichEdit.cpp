...
#define HIMETRIC_PER_INCH 2540
	...
	HRESULT InitDefaultCharFormat(CRichEditUI* re, CHARFORMAT2W* pcf, HFONT hfont)
	{
		...
		if (re->GetManager()->GetBackgroundTransparent())
		{
	...
	BOOL CTxtWinHost::Init(CRichEditUI *re, const CREATESTRUCT *pcs)
	{
		...
		// 启用编辑框其他功能 Add by Aries 2016/10/15
		if (true)
		{
			LRESULT lMask;
			pserv->TxSendMessage(EM_GETEVENTMASK, 0, 0, &lMask);
			pserv->TxSendMessage(EM_SETEVENTMASK, 0, lMask | ENM_CHANGE, &lMask);		// 启用文本改变通知的消息
			pserv->TxSendMessage(EM_GETEVENTMASK, 0, 0, &lMask);
			pserv->TxSendMessage(EM_SETEVENTMASK, 0, lMask | ENM_LINK, &lMask);			// 启用带Link文本的事件通知

			// 该属性不知道为什么无效(暂时只能在MessageHandler函数中拦截"复制"、"粘贴"快捷键消息)
			//pserv->TxSendMessage(EM_GETEVENTMASK, 0, 0, &lMask);						
			//pserv->TxSendMessage(EM_SETEVENTMASK, 0, lMask | ENM_KEYEVENTS, &lMask);	// 启用键盘的事件通知

			::RegisterClipboardFormat(CF_RTF);											// 注册剪切板为纯文本
		}
	...
	{
		if (m_re->GetManager()->GetBackgroundTransparent())
		{
	...
	BOOL CTxtWinHost::TxShowCaret(BOOL fShow)
	{
		if (m_re->GetManager()->GetBackgroundTransparent())
		{
			...
			{
				if (((m_re->IsReadOnly() && !m_re->IsReadOnlyShowCaret())
					|| !m_re->Activate()))
				{
	...
	void CRichEditUI::SetAlign(DWORD iAlign)
	{
		if (m_pTwh)
			m_pTwh->SetDefaultAlign(iAlign);
	}

	void CRichEditUI::SetPassword(bool bPassword /* = false */)
	{
		if (m_pTwh)
		{
			WCHAR chPasswordChar = 0;
	...
	bool CRichEditUI::IsReadOnlyShowCaret()
	{
		return m_bReadOnlyShowCaret;
	}

	void CRichEditUI::SetReadOnlyShowCaret(bool bShowCaret /* = true */)
	{
		m_bReadOnlyShowCaret = bShowCaret;
	}

	void CRichEditUI::SetCanPaste(bool bCanPaste /* = true */)
	{
		m_bCanPaste = bCanPaste;
	}

	bool CRichEditUI::IsCanPaste()
	{
		return m_bCanPaste;
	}
	...
	{
		if (!m_bInputMode && _tcscmp(m_sText, _T("")) == 0)
			return _T("");
	...
	{
		if (!m_pTwh || !pstrText) return;

		m_sText = pstrText;
		bool bRes1 = !IsFocused() && _tcscmp(m_sText, _T("")) == 0;
		bool bRes2 = !m_bInputMode && _tcscmp(pstrText, _T("")) == 0;
		if ((bRes1 || bRes2) && m_sTipValue.GetLength() > 0)
		{
			m_bInputMode = false;
			SetTipValue(m_sTipValue);
		}
		else
		{
			m_bInputMode = true;
	...
	{
		if (m_pManager->GetBackgroundTransparent())
		{
	...
	void CRichEditUI::Paste()
	{
		...
		this->GetClipboardText(pzCopyText);

		// 设置选中
		CHARRANGE cr;
		this->GetSel(cr);
		this->SetSel(cr.cpMin, cr.cpMax);
		
		int iLeftWordsLen = 0;
		CDuiString pzLastText = this->GetText();
#ifdef UNICODE || _UNICODE
		iLeftWordsLen = pzLastText.GetLength() - (cr.cpMax - cr.cpMin);
#else
		int iLen = pzLastText.GetLength() + 1;
		LPWSTR lpwText = new WCHAR[iLen];
		::ZeroMemory(lpwText, iLen * sizeof(WCHAR));
		::MultiByteToWideChar(CP_ACP, 0, pzLastText, -1, (LPWSTR)lpwText, iLen);
		iLeftWordsLen = wcslen(lpwText) - (cr.cpMax - cr.cpMin);
		delete[] lpwText;
#endif // UNICODE || _UNICODE

		// 截取剩余字符
		iLeftWordsLen = m_iLimitText - iLeftWordsLen;
		if (iLeftWordsLen > 0)
		{
#ifdef UNICODE || _UNICODE
			int iCopyLen = min(pzCopyText.GetLength(), iLeftWordsLen);
			pzCopyText = pzCopyText.Left(iCopyLen);
#else
			int iCopyLen = pzCopyText.GetLength() + 1;
			LPWSTR lpwCopyText = new WCHAR[iCopyLen];
			::ZeroMemory(lpwCopyText, iCopyLen * sizeof(WCHAR));
			::MultiByteToWideChar(CP_ACP, 0, pzCopyText.GetData(), -1, lpwCopyText, iCopyLen);
			iCopyLen = wcslen(lpwCopyText);
			iCopyLen = min(iCopyLen, iLeftWordsLen) + 1;

			LPWSTR lpwTmp = new WCHAR[iCopyLen];
			::ZeroMemory(lpwTmp, iCopyLen * sizeof(WCHAR));
			::memcpy(lpwTmp, lpwCopyText, (iCopyLen - 1) * sizeof(WCHAR));
			delete[] lpwCopyText;

			iCopyLen = WideCharToMultiByte(CP_OEMCP, NULL, lpwTmp, -1, NULL, 0, NULL, FALSE);
			LPSTR lpOutTxt = new CHAR[iCopyLen];
			::WideCharToMultiByte(CP_OEMCP, 0, lpwTmp, -1, lpOutTxt, iCopyLen, NULL, NULL);
			pzCopyText = lpOutTxt;
			delete[] lpwTmp;
			delete[] lpOutTxt;
#endif // UNICODE || _UNICODE
			...
			//替换选中字符
			this->ReplaceSel(pzCopyText, TRUE);
		}
	}

	void CRichEditUI::GetClipboardText(CDuiString& pzText)
	{
		// 判断剪贴板的数据格式是否可以处理。
	...
	void CRichEditUI::SetClipboardText(CDuiString& pzText)
	{
		// 打开剪贴板
		if (!OpenClipboard(m_pManager->GetPaintWindow()))
		{
		...
		// 申请剪贴板空间
		int dwBytes = (pzText.GetLength() + 1) * sizeof(TCHAR);
		HGLOBAL hClipBuffer = ::GlobalAlloc(GMEM_MOVEABLE | GMEM_DDESHARE, dwBytes);
		if (hClipBuffer == NULL)
		{
			::CloseClipboard();
			return;
		...
		// 给申请的空间加锁
		TCHAR* cbuff = (TCHAR*)::GlobalLock(hClipBuffer);
		if (cbuff == NULL)
		{
			::GlobalFree(cbuff);
			::CloseClipboard();
		...
		// 拷贝文本内容到剪贴板
		_tcscpy(cbuff, LPTSTR(pzText.GetData()));

		// 操作完成，释放锁
		::GlobalUnlock(hClipBuffer);

		// 设置剪切板内容
		if (NULL == ::SetClipboardData(CF_TEXT, hClipBuffer))
		{
	...
	BOOL CRichEditUI::CanPaste(UINT nFormat/* = 0*/)
	{
		...
		//if (NULL == m_pTwh) return FALSE;
		//ITextServices* pserv = m_pTwh->GetTextServices();
		//if (NULL == pserv) return FALSE;
		//pserv->TxSendMessage(EM_CANPASTE, nFormat, 0L, &lRes);

		HRESULT lRes = 0;
		TxSendMessage(EM_CANPASTE, nFormat, 0, &lRes);

		return (BOOL)lRes;
	}

	void CRichEditUI::PasteSpecial(UINT uClipFormat, DWORD dwAspect /* = DVASPECT_CONTENT */, HMETAFILE hMF /* = 0 */)
	{
		//if (NULL == m_pTwh) return;
		//ITextServices* pserv = m_pTwh->GetTextServices();
		//if (NULL == pserv) return;
	...
	BOOL CRichEditUI::SetOleCallback(IRichEditOleCallback* pCallback)
	{
		...
		HRESULT lRes = 0;
		TxSendMessage(EM_SETOLECALLBACK, 0, (LPARAM)pCallback, &lRes);
		return lRes;
	}
	...
	void CRichEditUI::OnTxNotify(DWORD iNotify, void *pv)
	{
		switch (iNotify)
		{
			case EN_CHANGE:
			{
				GetManager()->SendNotify(this, DUI_MSGTYPE_TEXTCHANGED, (WPARAM)pv, (LPARAM)pv);
			}
			break;
			case EN_DROPFILES:
			case EN_MSGFILTER:
	...
	void CRichEditUI::DoEvent(TEventUI& event)
	{
		...
		if (event.Type == UIEVENT_SETFOCUS) {
			...
			if (m_pTwh) {
				...
				if (!m_bInputMode)
				{
					...
					// 如果有tipvalue, 则重新聚焦时清除tipvalue文本(否则不清除文本)
					if (_tcscmp(GetTipValue(), _T("")) != 0)
						SetText(_T(""));
				}
				//modify by Aries ------
		...
		if (event.Type == UIEVENT_KILLFOCUS)  {
			...
			if (m_pTwh) {
				...
				//modify by Aries +++++++
				if (m_bInputMode && GetText() == _T("")
					&& (_tcscmp(GetTipValue(), _T("")) != 0))	// 如果有tipvalue,则显示tipvalue(否则正常显示)
				{
					m_bInputMode = false;
					SetTipValue(m_sTipValue);
				}
	...
	void CRichEditUI::SetTipValue(LPCTSTR pStrTipValue)
	{
		...
		if (m_pTwh && !m_bInputMode && !m_sTipValue.IsEmpty())
		{
			SetSel(0, -1);
			m_pTwh->SetColor(m_dwTipValueColor);
			ReplaceSel(m_sTipValue, FALSE);
		}
	}
	...
	void CRichEditUI::SetAttribute(LPCTSTR pstrName, LPCTSTR pstrValue)
	{
		...
		else if (_tcscmp(pstrName, _T("readonlyshowcaret")) == 0) {
			m_bReadOnlyShowCaret = _tcscmp(pstrValue, _T("true")) == 0;
		}
		else if (_tcscmp(pstrName, _T("canpaste")) == 0)
		{ 
			m_bCanPaste = _tcscmp(pstrValue, _T("true")) == 0;
		}
	...
	LRESULT CRichEditUI::MessageHandler(UINT uMsg, WPARAM wParam, LPARAM lParam, bool& bHandled)
	{
		...
		if (uMsg == WM_IME_COMPOSITION)
		{
			...
			if (hIMC)
			{
				...
				//	ImmGetCompositionString(hIMC, GCS_RESULTSTR, pzBuff, dwSize);
				//}
			...
			if (!m_pTwh->IsCaptured()) {
				switch (uMsg) {
					...
					case WM_RBUTTONUP:
					{
						...
						{
							GetManager()->SendNotify(this, DUI_MSGTYPE_RDOWN_RICHEDIT, wParam, lParam);
						}
					}
					break;
				}
		...
		// 拦截 "复制CTRL+C"、"粘贴CTRL+V"、"撤销CTRL+Z"、"反撤销CTRL+Y"
		// Add by Aries 2016/10/15
		// ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
		if (WM_KEYDOWN == uMsg)
		{
			if (this->IsFocused() && (GetKeyState(VK_CONTROL) < 0) && (wParam == 'C'))
			{
				this->Copy();
				return 0;
			}
			else if (this->IsFocused() && (GetKeyState(VK_CONTROL) < 0) && (wParam == 'V'))
			{
				if (m_bCanPaste && !m_bReadOnly)
				{
					this->Paste();
				}
			...
			else if (this->IsFocused() && (GetKeyState(VK_CONTROL) < 0) && (wParam == 'X'))
			{
				this->Cut();
				return 0;
			}
			else if (this->IsFocused() && (GetKeyState(VK_CONTROL) < 0) && (wParam == 'Z'))
			{
				this->Undo();
				return 0;
			}
			else if (this->IsFocused() && (GetKeyState(VK_CONTROL) < 0) && (wParam == 'Y'))
			{
				this->Redo();
				return 0;
			}
	...
	//是否是输入模式(判断是否是输入文本, 还是提示文本)
	bool CRichEditUI::IsInputMode()
	{
		return m_bInputMode;
	}

	CTxtWinHost* CRichEditUI::GetTxtWinHost()
	{
		return m_pTwh;
	}

	IRichEditOle* CRichEditUI::GetRichEditOle()
	{

...
bool CListUI::Add(CControlUI* pControl)
{
    ...
    if( _tcsstr(pControl->GetClass(), _T("ListHeaderItemUI")) != NULL ) {
		...
		CListHeaderItemUI* pListHeaderItem = static_cast<CListHeaderItemUI*>(pControl);
		DWORD w = pListHeaderItem->GetSepWidth();
        m_ListInfo.nColumns = MIN(m_pHeader->GetCount(), UILIST_MAX_COLUMNS);
        return ret;
    }
...
void CListUI::SetPos(RECT rc)
{
    ...
    for( int i = 0; i < m_ListInfo.nColumns; i++ ) {
		...
		CListHeaderItemUI* pListHeaderItemUI = dynamic_cast<CListHeaderItemUI*>(pControl);
		if (pListHeaderItemUI != NULL)
		{
			DWORD iSeWidth = pListHeaderItemUI->GetSepWidth();
			m_ListInfo.iSepWidth[i] = iSeWidth;
		}
...
void CListUI::SetScrollRange(SIZE szRange)
{
	m_pList->SetScrollRange(szRange);
}
...
//此函数有改动,修复滚动条滚动后,点击List伸缩时位置被重置的bug
...
void CListHeaderItemUI::PaintText(HDC hDC)
{
    ...
    if( m_bShowHtml )
		...
		CRenderEngine::DrawCustomHtmlText(hDC, m_pManager, rcText, m_sText, m_dwTextColor, \
...
bool CListElementUI::Select(bool bSelect)
{
    ...
    //if( bSelect == m_bSelected ) return true;		// Modify by Aries 2017.04.13 其所属者可以自己Selected自己
    m_bSelected = bSelect;
...
SIZE CListLabelElementUI::EstimateSize(SIZE szAvailable)
{
    ...
    if( cXY.cx == 0 && m_pManager != NULL ) {
        ...
        if( pInfo->bShowHtml ) {
			...
			CRenderEngine::DrawCustomHtmlText(m_pManager->GetPaintDC(), m_pManager, rcText, m_sText, 0, pInfo->nFont, DT_SINGLELINE | DT_CALCRECT | pInfo->uTextStyle & ~DT_RIGHT & ~DT_CENTER);
		}
        else {
...
void CListLabelElementUI::DrawItemText(HDC hDC, const RECT& rcItem)
{
    ...
    if( pInfo->bShowHtml )
		...
		CRenderEngine::DrawCustomHtmlText(hDC, m_pManager, rcText, m_sText, iTextColor, \
...
void CListTextElementUI::DrawItemText(HDC hDC, const RECT& rcItem)
{
    ...
    for( int i = 0; i < pInfo->nColumns; i++ )
    {
		...
		//减去ListHeaderItem之间的间距(即sepwidth)
		rcItem.right -= pInfo->iSepWidth[i];
		
		// Modify by Aries 2016/12/3
		// 这里请使用指针
		// 因为当传出的字符串过长时,在局部赋值过程中，会重新分配内存(可能是异步的)
		// 然后传入DrawCustomHtmlText里面时，内存部分丢失，虽然不会崩溃，但是会引起异常
		// 用指针安全点
		// 总之都TMD的是坑
		LPCTSTR lpText = nullptr;
		if (pCallback)
		{
			lpText = pCallback->GetItemText(this, m_iIndex, i);
		}
		else
		{
			lpText = GetText(i);
		...
		if (pInfo->bShowHtml)
		{
			CRenderEngine::DrawCustomHtmlText(hDC, m_pManager, rcItem, lpText, iTextColor, \
				pInfo->nFont, DT_SINGLELINE | pInfo->uTextStyle);
		}
		else
		{
			CRenderEngine::DrawText(hDC, m_pManager, rcItem, lpText, iTextColor, \
				pInfo->nFont, DT_SINGLELINE | pInfo->uTextStyle);
		}

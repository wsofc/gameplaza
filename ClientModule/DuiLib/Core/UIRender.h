...
public:
    ...
    static void DrawImage(HDC hDC, HBITMAP hBitmap, const RECT& rc, const RECT& rcPaint, \
		...
		bool hole = false, bool xtiled = false, bool ytiled = false, bool bAligenCenter = false, bool bGray = false);
	...
	static void DrawFocusRoundRect(HDC hDC, const RECT& rc, int nSize, DWORD dwPenColor);
    static void DrawText(HDC hDC, CPaintManagerUI* pManager, RECT& rc, LPCTSTR pstrText, \
        DWORD dwTextColor, int iFont, UINT uStyle);
	static void DrawCustomHtmlText(HDC hDC, CPaintManagerUI* pManager, RECT& rc, LPCTSTR pstrText, \
		DWORD dwTextColor, int iFont, UINT uStyle);
	...
	// Add by Aries 2017/12/09
	// ----------------------------------------------------------- zip相关操作 -----------------------------------------------------------
	static bool IsZipItemExist(LPCTSTR lpszPath);															// 判断zip中子文件是否存在
	static DWORD GetZipItemSize(LPCTSTR lpszPath);															// 获取zip中子文件大小
	static bool GetZipItemData(LPCTSTR lpszPath, char* pOutData);											// 获取zip中子文件字节流
	static int GetZipItemIndex(LPCTSTR lpszPath);															// 获取zip中子文件索引
	static bool TraverseZipFolder(LPCTSTR lpszPath, int& iIndex, CDuiString& pOutData, DWORD& dwSize);		// 遍历zip目录中子文件列表
};

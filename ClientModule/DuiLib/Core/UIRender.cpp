...
namespace DuiLib {
	...
	void CRenderEngine::DrawImage(HDC hDC, HBITMAP hBitmap, const RECT& rc, const RECT& rcPaint
		...
		, bool xtiled /* = false */, bool ytiled /* = false */, bool bAligenCenter /* = false */, bool bGray /* = false */)
	{
		ASSERT(::GetObjectType(hDC) == OBJ_DC || ::GetObjectType(hDC) == OBJ_MEMDC);
		...
		{
			BITMAP bm;
			if (FALSE != ::GetObject(hBitmap, sizeof(bm), &bm))
			{
				// 定义位图信息
				BITMAPINFO bi;
				...
				// 获取位图数据
				int linebytes = bi.bmiHeader.biSizeImage;
				BYTE* pBits = (BYTE*)new BYTE[linebytes];
				::ZeroMemory(pBits, linebytes);
				if (::GetDIBits(hCloneDC, hBitmap, 0, bm.bmHeight, pBits, &bi, DIB_RGB_COLORS))
				{
					for (int i = 0; i < linebytes; i += 4)
					{
						...
						// 核心灰度算法
						int val = r * 0.114 + g * 0.587 + b * 0.299;
					...
					// 创建新位图，设置位图数据
					hGray = ::CreateCompatibleBitmap(hCloneDC, bm.bmWidth, bm.bmHeight);
					::SetDIBits(hCloneDC, hGray, 0, bm.bmHeight, pBits, &bi, DIB_RGB_COLORS);
					::SelectObject(hCloneDC, hGray);
				}
		...
		if (lpAlphaBlend && (alphaChannel || uFade < 255)) {
			...
			if (!hole) {
				...
				if (::IntersectRect(&rcTemp, &rcPaint, &rcDest)) {
					...
					//此处添加原图居中绘制的属性(会优先判断，如果为真则会过滤掉之后的xtiled 和 有ytiled 等属性)
					//添加本意为: 按钮上可以添加一个图片, 且可以原图居中,若要其他对齐方式,具体需求在编写
					//但是写在此处可以通用，直接在xml中的相应 image字段里添加"aligncenter=true"字段即可使之生效与否(默认值false)
					//modify by King +++++++++++++++++++++++++++
					if (bAligenCenter)
					{
						//目标区域宽高
						...
						//原位图宽高
						LONG rcBmp_w = rcBmpPart.right - rcBmpPart.left;
						LONG rcBmp_h = rcBmpPart.bottom - rcBmpPart.top;

						//是否是等比压缩模式(目前需求来，小图大小不变居中，大图最大化压缩居中)
						BOOL bEqual_Compress_Mode = true;
						if (bEqual_Compress_Mode)
						{
							//先压缩，后居中
							float sx = rcBmp_w / (float)rcDest.right;
							float sy = rcBmp_h / (float)rcDest.bottom;
							float s_max = sx > sy ? sx : sy;
							if (sx > 1 || sy > 1)
							{
								rcBmp_w /= s_max;
								rcBmp_h /= s_max;
						...
						//原图居中计算
						rcDest.left += (rcDest.right - rcBmp_w) / 2;
						rcDest.top += (rcDest.bottom - rcBmp_h) / 2;
						rcDest.right = rcBmp_w;
						rcDest.bottom = rcBmp_h;
	...
	bool DrawImage(HDC hDC, CPaintManagerUI* pManager, const RECT& rc, const RECT& rcPaint, const CDuiString& sImageName, \
		...
		bool bHole, bool bTiledX, bool bTiledY, bool bAlignCenter = false, bool bGray = false)
	{
		...
		CRenderEngine::DrawImage(hDC, data->hBitmap, rcItem, rcPaint, rcBmpPart, rcCorner, pManager->GetBackgroundTransparent() ? true : data->alphaChannel, bFade, bHole, bTiledX, bTiledY, bAlignCenter, bGray);
		...
		LPCTSTR pStrImage, LPCTSTR pStrModify)
	{
		...
		for (int i = 0; i < 2; ++i, image_count = 0) {
			...
			while (*pStrImage != _T('\0')) {
				...
				if (!sValue.IsEmpty()) {
					...
					else if (sItem == _T("aligncenter")) {
						bAlignCenter = (_tcscmp(sValue.GetData(), _T("true")) == 0);
					}
		...
		DuiLib::DrawImage(hDC, pManager, rc, rcPaint, sImageName, sImageResType,
			rcItem, rcBmpPart, rcCorner, dwMask, bFade, bHole, bTiledX, bTiledY, bAlignCenter, bGray);
	...
	// 绘制虚线边框 Add by Aries 2016/10/08
	void CRenderEngine::DrawFocusRoundRect(HDC hDC, const RECT& rc, int nSize, DWORD dwPenColor)
	{
		// Tab键聚焦时，绘制虚线边框,用于切换控件提示

		// 由于GDI画刷不能透明等问题，所以不用DrawRoundRect函数
		// 而采用此函数用GDI+来实现

		// 方案一(可自定义色彩,线宽等):
		Gdiplus::Graphics graphics(hDC);
		Gdiplus::Pen hPen(Gdiplus::Color(GetBValue(dwPenColor), GetGValue(dwPenColor), GetRValue(dwPenColor)), nSize);
		hPen.SetDashStyle(Gdiplus::DashStyleDash);
		Gdiplus::HatchBrush hBrush(Gdiplus::HatchStyleCross, Gdiplus::Color(0, 255, 0), Gdiplus::Color(0, 0, 255));
		const Gdiplus::Rect gdi_rc = { rc.left, rc.top, rc.right - rc.left - 1, rc.bottom - rc.top - 1 };
		graphics.DrawRectangle(&hPen, gdi_rc);

		//方案二(采用API,无法自定义):
		//if (IsFocused() && m_dwFocusBorderColor != 0)
		//{
		//	::DrawFocusRect(hDC, &m_rcItem);
		//}
	}
	...
	void CRenderEngine::DrawText(HDC hDC, CPaintManagerUI* pManager, RECT& rc, LPCTSTR pstrText, DWORD dwTextColor, int iFont, UINT uStyle)
	{
		...
		if (pManager->GetBackgroundTransparent() || pManager->IsUseGdiplusText())
		{
			Gdiplus::Graphics graphics(hDC);
			...
			// Modify by Aries 2017/03/20
			// 此处"&"上的Alpha值改为254(0xFE)，同时避免分层窗口透明背景下字体穿透和变形的问题
			// 至于为什么不是255而是254，一直疑惑，暂未深究其因...
			// 其实有三种方案，但是都会导致字体变味：加粗了、抗锯齿了、变形了
			// 方案一:字体加宽
			// 方案二:SetTextRenderingHint(TextRenderingHintAntiAlias)
			// 方案三:把字串转换成GraphicsPath，然后用FillPath画出来(画之前SetSmoothingMode(SmoothingModeHighQuality)字会好看一点)
			// 因此采用最终方案"& 0xFE"
			DWORD dwAlpha = dwTextColor & 0xFE000000;
			BYTE cbAlpha = LOBYTE(dwAlpha >> 24);
			Gdiplus::SolidBrush brush(Gdiplus::Color(cbAlpha, GetBValue(dwTextColor), GetGValue(dwTextColor), GetRValue(dwTextColor)));
			...
			#else
				WCHAR* wpstrText = nullptr;
				size_t iLen = ::MultiByteToWideChar(CP_ACP, 0, pstrText, -1, NULL, 0);
				wpstrText = new WCHAR[iLen];
				::MultiByteToWideChar(CP_ACP, 0, pstrText, -1, wpstrText, iLen);
				graphics.MeasureString(wpstrText, -1, &font, rectF, &stringFormat, &bounds);
				delete wpstrText;
			#endif // UNICODE
			...
			else
			{
				...
				wpstrText = new WCHAR[iLen];
				::MultiByteToWideChar(CP_ACP, 0, pstrText, -1, wpstrText, iLen);
				graphics.DrawString(wpstrText, -1, &font, rectF, &stringFormat, &brush);
				delete wpstrText;
			#endif // UNICODE
			}
	...
	void CRenderEngine::DrawCustomHtmlText(HDC hDC, CPaintManagerUI* pManager, RECT& rc, LPCTSTR pstrText, DWORD dwTextColor, int iFont, UINT uStyle)
	{
		//如果使用的是透明异形窗口(即xml中 bktans="true")
		//则使用用户自定义的文本绘制( DrawHtmlText 功能类似，只不过采用了GDI+绘制文本)
		//具体功能需求,具体添加
		//以下按照具体的需求定制的

		//格式为:
		//<bkimage file='xxx.png' aligncenter='true'>				图片xxx
		//<text content='xxx' font='x' textcolor='#FFFFFFFF'>		文本xxx
		//<bkcolor xxx>												颜色xxx
		// ... 
		//可嵌套(绘制顺序即为嵌套顺序)
		//<image=''><text=''><bkcolor=''>
		...
		while (*pstrText != _T('\0')) {
			...
			while (*pstrText != _T('\0') && *pstrText != _T(' ') && *pstrText != _T('>')) {
				...
				while (pstrText < pstrTemp) sItem += *pstrText++;
			}
			...
			if (sItem == _T("bkimage"))
			{
				...
				while (*pstrText != _T('\0') && *pstrText != _T('>')) {
					LPTSTR pstrTemp = ::CharNext(pstrText);
					while (pstrText < pstrTemp)	sValue += *pstrText++;
				}

				CRenderEngine::DrawImageString(hDC, pManager, rc, rc, sValue);
			}
			else if (sItem == _T("text"))
			{
				CDuiString sContent;
				CDuiString sFont;
				CDuiString sTextColor;
				...
				while (*pstrText != _T('\0'))
				{
					...
					// 跳过'\''后移至有效字符
					while (*pstrText != _T('\0') && *pstrText != _T('\'')) pstrText = ::CharNext(pstrText);
					while (*pstrText != _T('\0')){ pstrText = ::CharNext(pstrText); break; }
					...
					if (!sValue.IsEmpty())
					{
						...
						else if (sItem == _T("font"))
						{
							sFont = sValue;
						}
						else if (sItem == _T("textcolor"))
						{
							sTextColor = sValue;
						}
					}
				...
				int iTempFont = iFont;
				DWORD dwTempTextColor = dwTextColor;
				if (!sFont.IsEmpty())
				{
					iTempFont = _ttoi(sFont.GetData());
				}
				if (!sTextColor.IsEmpty())
				{
					//LPTSTR endptr = nullptr;
					//dwTempTextColor = _tcstoll(sTextColor.GetData(), const_cast<LPTSTR*>(&endptr), 16);

					dwTempTextColor = _ttoll(sTextColor.GetData());
				}

				CRenderEngine::DrawText(hDC, pManager, rc, sContent, dwTempTextColor, iTempFont, uStyle);
			}
			else if (sItem == _T("bkcolor"))
			{
				while (*pstrText != _T('\0') && *pstrText == _T(' ')) pstrText = ::CharNext(pstrText);
				while (*pstrText != _T('\0') && *pstrText != _T('>')) {
					LPTSTR pstrTemp = ::CharNext(pstrText);
				...
				LPTSTR pstr = NULL;
				DWORD bkColor = _tcstoul(sValue, &pstr, 16);
				CRenderEngine::DrawColor(hDC, rc, bkColor);
			}
			else if (!sItem.IsEmpty())
			{
				...
				CRenderEngine::DrawText(hDC, pManager, rc, sItem, dwTextColor, iFont, uStyle);
			}

			if (sItem.IsEmpty() || *pstrText == _T('>')) pstrText = ::CharNext(pstrText);
		}
	}
	...
	bool CRenderEngine::IsZipItemExist(LPCTSTR lpszPath)
	{
		if (!lpszPath || _tcscmp(lpszPath, _T("")) == 0) return false;
		CDuiString sFile = CPaintManagerUI::GetResourcePath();
		sFile += CPaintManagerUI::GetResourceZip();
		...
		ZIPENTRY ze;
		int i;
		if (FindZipItem(hz, lpszPath, true, &i, &ze) != 0) return false;
	...
	DWORD CRenderEngine::GetZipItemSize(LPCTSTR lpszPath)
	{
		if (!lpszPath || _tcscmp(lpszPath, _T("")) == 0) return 0;
		CDuiString sFile = CPaintManagerUI::GetResourcePath();
		sFile += CPaintManagerUI::GetResourceZip();
		...
		ZIPENTRY ze;
		int i;
		if (FindZipItem(hz, lpszPath, true, &i, &ze) != 0) return 0;
		if (!CPaintManagerUI::IsCachedResourceZip()) CloseZip(hz);

		return ze.unc_size;
	}
	
	bool CRenderEngine::GetZipItemData(LPCTSTR lpszPath, char* pOutData)
	{
		if (!lpszPath || _tcscmp(lpszPath, _T("")) == 0 || !pOutData) return false;
		CDuiString sFile = CPaintManagerUI::GetResourcePath();
		sFile += CPaintManagerUI::GetResourceZip();
		...
		DWORD dwSize = ze.unc_size;
		if (dwSize == 0) return false;
		int res = UnzipItem(hz, i, pOutData, dwSize, 3);
		if (res != 0x00000000 && res != 0x00000600)
		{
			if (!CPaintManagerUI::IsCachedResourceZip()) CloseZip(hz);
			return false;
	...
	int CRenderEngine::GetZipItemIndex(LPCTSTR lpszPath)
	{
		if (!lpszPath || _tcscmp(lpszPath, _T("")) == 0) return 0;
		CDuiString sFile = CPaintManagerUI::GetResourcePath();
		sFile += CPaintManagerUI::GetResourceZip();
		...
		do
		{
			...
			ZIPENTRY ze;
			int i;
			if (FindZipItem(hz, lpszPath, true, &i, &ze) != 0) break;
			iIndex = i;
		} while (0);
	...
	bool CRenderEngine::TraverseZipFolder(LPCTSTR lpszPath, int& iIndex, CDuiString& pOutData, DWORD& dwSize)
	{
		if (!lpszPath || _tcscmp(lpszPath, _T("")) == 0) return false;
		CDuiString sFile = CPaintManagerUI::GetResourcePath();
		sFile += CPaintManagerUI::GetResourceZip();
		...
		do
		{
			...
			ZIPENTRY ze;
			if (0 == GetZipItem(hz, iIndex, &ze))
			{
				CDuiString pzDirPath = lpszPath;
				CDuiString pzFileName = ze.name;
				pzDirPath.Replace(_T("\\"), _T("/"));
				pzFileName.Replace(_T("\\"), _T("/"));
				int ipos = pzFileName.ReverseFind('/');
				if (ipos != -1)
				{
					pzFileName = pzFileName.Left(ipos + 1);
				}

				if (pzDirPath == pzFileName)
				{
					pOutData = ze.name;
					dwSize = ze.comp_size;
					bRes = true;
		...
		//// 若不是目录则返回
		//if ((ze.attr & FILE_ATTRIBUTE_DIRECTORY) == 0) return false;

		//CDuiString pzDirPath = ze.name;
		//while (true)
		//{
		//	
		//	{

...
namespace DuiLib {
	...
	void CControlUI::SetFocusBorderOnDraw(bool bOnDraw)
	{
		m_bFocusBorderOnDraw = bOnDraw;
	}
	...
	void CControlUI::SetVoidData(void* pData)
	{
		m_pVoidData = pData;
	}

	const void* CControlUI::GetVoidData()
	{
		return m_pVoidData;
	}
	...
	{
		// ++++++++++++++++++++++++++++++++++++++++++
		// Modified by Aries 2016/09/28
		CControlUI* pParent = GetParent();
		if (pParent)
		{
			return pParent->IsKeyboardEnabled();
		}
		// -----------------------------------------
	...
	void CControlUI::SetTabEnable(bool bEnable /* = false */)
	{
		m_bTabEnable = bEnable;
	}

	bool CControlUI::IsTabEnable()
	{
		return m_bTabEnable;
	}
	...
	void CControlUI::RestState(UINT state /* = 0 */)
	{
	...
	void CControlUI::SetAttribute(LPCTSTR pstrName, LPCTSTR pstrValue)
	{
		...
		else if (_tcscmp(pstrName, _T("tab")) == 0) SetTabEnable(_tcscmp(pstrValue, _T("true")) == 0);
	...
	void CControlUI::PaintBorder(HDC hDC)
	{
		...
		{
			if (m_nBorderSize > 0)	// Ìí¼Ó¾Û½¹ÐéÏß±ß¿òÌáÊ¾	Add by Arries 2016/10/08
			{
				if (IsFocused() && m_bFocusBorderOnDraw && m_dwFocusBorderColor != 0)
				{
					CRenderEngine::DrawFocusRoundRect(hDC, m_rcItem, m_nBorderSize, GetAdjustColor(m_dwFocusBorderColor));
				}
			}
			/*

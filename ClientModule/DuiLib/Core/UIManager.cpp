...
namespace DuiLib {
	...
	void CPaintManagerUI::ReloadZipResource(int ID)
	{
		HRSRC hResource = ::FindResource(GetResourceDll(), MAKEINTRESOURCE(ID), _T("ZIPRES"));
	...
	void CPaintManagerUI::ClearResourceZip()
	{
	...
	{
		if ((x < 0 || y < 0) || (FALSE == ::SetCaretPos(x, y)))
		{
			return false;
		}
	...
	bool CPaintManagerUI::PreMessageHandler(UINT uMsg, WPARAM wParam, LPARAM lParam, LRESULT& /*lRes*/)
	{
		...
		switch (uMsg) {
			case WM_KEYDOWN:
			{
				...
				if (wParam == VK_TAB)
				{
					if (m_pFocus && m_pFocus->IsKeyboardEnabled()) // Modified by Aries 2016/09/28
					{
						// 编辑框是否接受掉Tab键消息(即Tab键被作为字符输入)
						if (m_pFocus->IsVisible() && m_pFocus->IsEnabled() && _tcsstr(m_pFocus->GetClass(), _T("RichEditUI")) != NULL) {
							if (static_cast<CRichEditUI*>(m_pFocus)->IsWantTab()) return false;
						}

						// 循环查找tab焦点(每次最对循环一遍)
						CControlUI* pNextTabCtl = m_pFocus;
						CContainerUI* pRootContainer = (CContainerUI*)m_pRoot->GetInterface(DUI_CTR_CONTAINER);
						if (pRootContainer)
						{
							DWORD dwCount = this->GetAllChildrens(m_pRoot);
							for (size_t i = 0; i < dwCount; ++i)
							{
								pNextTabCtl = this->FindNextTabControl(pNextTabCtl, ::GetKeyState(VK_SHIFT) >= 0);
								if (pNextTabCtl)
								{
									if (m_pFocus == pNextTabCtl || pNextTabCtl->IsTabEnable())
									{
										if (pNextTabCtl->IsTabEnable())
										{
											pNextTabCtl->SetFocusBorderOnDraw(true);

											SetFocus(pNextTabCtl);
											m_bFocusNeeded = false;
										}
		...
		switch (uMsg) {
			...
			case WM_PAINT:
			{
				...
				if (m_bAlphaBackground)
				{
					...
					// ----------------------------------------------------------------------------
					// Add by Arise 2017/02/10
					// 修复电脑锁屏后恢复屏幕刷新m_pBmpOffscreenBits越界崩溃的bug
					// 锁屏后本次需要刷新的rcPaint区域并未刷新，还保留上次的区域
					// 因此有交集则取交集，无交集则取rcClient,完美解决bug
					if (FALSE == ::IntersectRect(&rcPaint, &rcClient, &rcPaint))
					{
						rcPaint = rcClient;
					}
					// ----------------------------------------------------------------------------
	...
	void CPaintManagerUI::ReapObjects(CControlUI* pControl)
	{
		...
		if (!sName.IsEmpty()) {
			if (m_pRoot && pControl == FindControl(sName)) m_mNameHash.Remove(sName);
		}
	...
	void CPaintManagerUI::SetFocus(CControlUI* pControl)
	{
		...
		if (m_pFocus != NULL)
		{
			...
			event.dwTimestamp = ::GetTickCount();
			m_pFocus->Event(event);
			m_pFocus->SetFocusBorderOnDraw(false);
	...
	CControlUI* CPaintManagerUI::FindNextTabControl(CControlUI* pLastCtl, bool bForward /* = true */)
	{
		// Find next/previous tabbable control
		CControlUI* pNextCtl = nullptr;
		FINDTABINFO info1 = { 0 };
		info1.pFocus = pLastCtl;
		info1.bForward = bForward;
		pNextCtl = m_pRoot->FindControl(__FindControlFromTab, &info1, UIFIND_VISIBLE | UIFIND_ENABLED | UIFIND_ME_FIRST);
		if (pNextCtl == NULL) {
			if (bForward) {
				// Wrap around
				FINDTABINFO info2 = { 0 };
				info2.pFocus = bForward ? NULL : info1.pLast;
				info2.bForward = bForward;
				pNextCtl = m_pRoot->FindControl(__FindControlFromTab, &info2, UIFIND_VISIBLE | UIFIND_ENABLED | UIFIND_ME_FIRST);
			}
			else {
				pNextCtl = info1.pLast;
			}
		}
	...
	bool CPaintManagerUI::UseParentResource(CPaintManagerUI* pm)
	{
		...
		//拷贝自定义字体资源
		int iCustomFontCount = pm->GetCustomFontCount();
		if (iCustomFontCount > 0)
		{
			this->m_aCustomFonts.Empty();
			for (int i = 0; i < iCustomFontCount; ++i)
			{
				TFontInfo* pTFontInfo = pm->GetFontInfo(i);
				this->AddFont(pTFontInfo->sFontName
					, pTFontInfo->iSize
					, pTFontInfo->bBold
					, pTFontInfo->bUnderline
					, pTFontInfo->bItalic);
			}
		}
	...
	DWORD CPaintManagerUI::GetAllChildrens(CControlUI* pControl) const
	{
		DWORD dwCount = 0;
		CContainerUI* pContainer = (CContainerUI*)pControl->GetInterface(DUI_CTR_CONTAINER);
		if (pContainer)
		{
			DWORD dwTempCount = pContainer->GetCount();
			dwCount += dwTempCount;
			for (size_t i = 0; i < dwTempCount; ++i)
			{
				dwCount += this->GetAllChildrens(pContainer->GetItemAt(i));
			}
		}
		return dwCount;
	}

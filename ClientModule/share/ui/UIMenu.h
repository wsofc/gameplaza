...
public:
	...
	// 设置菜单项生效与否
	void SetMenuItemEnable(LPCTSTR lpszName, BOOL bEnable);
...
public:
	...
	CControlUI *	m_pParentControl;		//依赖的父控件
	CDuiString		m_pzNodeName;
};
...
public:
	...
	//添加多项单选(类似 COptionUI 中的 "group" 功能)
	//Modify by Aries 2016/04/29
	LPCTSTR GetGroup() const;
	void SetGroup(LPCTSTR pStrGroupName = NULL);
	void SetChecked(bool bCheck, bool bGroupSet = false);
	bool GetChecked() const;
	void SetCheckSwitch(bool bCheckItem = false);
	bool GetCheckSwitch() const;
...
protected:
	...
	SIZE		m_szIconSize; 		// 图标大小
	CDuiString	m_strIcon;			// 图标路径
	bool		m_bCheckSwitch;		// 选择开关
	CDuiString	m_sGroupName;		// 选择项所属组名(等同于COptionUI的"group")

	bool		m_bShowExplandIcon;	// 是否有展开图标
};

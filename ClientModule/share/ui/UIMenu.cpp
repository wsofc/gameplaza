...
BOOL CMenuWnd::Receive(ContextMenuParam param)
{
	switch (param.wParam)
	{
		...
		{
			Close();
			if (param.lParam != NULL)
			{
				CPaintManagerUI* pManager = CMenuWnd::GetGlobalContextMenuObserver().GetManager();
				if (pManager)
				{
					LPCTSTR lpStr = (LPCTSTR)(param.lParam);
					CDuiString* pMenuElementStr = new CDuiString(lpStr); // 在处理菜单按钮消息时,记得delete掉(名字)
					if (!::PostMessage(pManager->GetPaintWindow(), WM_MENUCLICK, (WPARAM)(pMenuElementStr), (LPARAM)(m_pParentControl)))
						delete pMenuElementStr;
				}
			}
		}
		break;
...
void CMenuWnd::Init(CMenuElementUI* pOwner
	...
	, CControlUI* pParentControl /* = NULL  */
...
LRESULT CMenuWnd::OnCreate(UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL& bHandled)
{
	if( m_pOwner != NULL) {
		...
		CControlUI* pRoot = builder.Create(m_xml, m_type, this, &m_pm);
		if (pRoot == NULL)
		{
...
void CMenuWnd::ResizeMenu()
{
	...
	// Add by Aries 2016/11/21
	// 添加自动适配菜单位置
	// 修复"任务栏横、纵向排列时菜单位置显示不真确的bug"
	//int with = GetSystemMetrics(SM_CXFULLSCREEN);
	//int heigh = GetSystemMetrics(SM_CYFULLSCREEN);
	//int iScreenWith = ::GetSystemMetrics(SM_CXVIRTUALSCREEN);
	//int iScreenHeigh = ::GetSystemMetrics(SM_CYVIRTUALSCREEN);

	// 获得工作区大小
	RECT rcDeskTop = { 0 };
	::SystemParametersInfo(SPI_GETWORKAREA, 0, &rcDeskTop, 0);

	LONG dif_x = rcDeskTop.right - point.x;
	LONG dif_y = rcDeskTop.bottom - point.y;
	do
	{
		DWORD dwAlign_X = eMenuAlignment_Left;
		DWORD dwAlign_Y = eMenuAlignment_Top;
		if (dif_x < szInit.cx)
		{
			dwAlign_X = eMenuAlignment_Right;
		}
		...
		if (dif_y < szInit.cy)
		{
			...
			{
				dwAlign_Y = eMenuAlignment_Adaptation;
			}
			else
			{
				dwAlign_Y = eMenuAlignment_Bottom;
			}
		}

		m_dwAlignment = dwAlign_X | dwAlign_Y;
	} while (0);
	...
	else if (m_dwAlignment & eMenuAlignment_Adaptation)
	{
		rc.top = point.y * (rcDeskTop.bottom - rcDeskTop.top) / nHeight - point.y;
	...
	::SetWindowPos(m_hWnd, HWND_TOPMOST, rc.left, rc.top,
		rc.GetWidth(), rc.GetHeight() + pMenuRoot->GetInset().bottom + pMenuRoot->GetInset().top, SWP_SHOWWINDOW);

	RECT rcFlush = { 0 };
	rcFlush.left = rc.left;
	rcFlush.top = rc.top;
	rcFlush.right = rc.right;
	rcFlush.bottom = rc.bottom;
...
void CMenuWnd::SetMenuItemEnable(LPCTSTR lpszName, BOOL bEnable)
{
...
CMenuElementUI* CMenuWnd::GetMenuItem(LPCTSTR lpszName)
{
	CMenuElementUI* pControl = nullptr;
	pControl = (CMenuElementUI*)m_pm.FindControl(lpszName);
...
CMenuElementUI::~CMenuElementUI()
{
	//析构时记得要移除相应的group容器
	CPaintManagerUI* pManager = CMenuWnd::GetGlobalContextMenuObserver().GetManager();
	if (!m_sGroupName.IsEmpty() && pManager) pManager->RemoveOptionGroup(m_sGroupName, this);
}
...
void CMenuElementUI::CreateMenuWnd()
{
	...
	m_pWindow->Init(static_cast<CMenuElementUI*>(this), _T(""), _T(""), CPoint(), NULL);
}
...
void CMenuElementUI::SetChecked(bool bCheck, bool bGroupSet /* = false */)
{
	std::map<CDuiString, bool>* pCheckInfo = CMenuWnd::GetGlobalContextMenuObserver().GetMenuCheckInfo();
	if (!m_bCheckSwitch || pCheckInfo == NULL)  return;

	std::map<CDuiString, bool>::iterator it = pCheckInfo->find(GetName());
	if (it == pCheckInfo->end())
		pCheckInfo->insert(std::map<CDuiString, bool>::value_type(GetName(), bCheck));
	else
	{
		if (bGroupSet) return;
		if (!m_sGroupName.IsEmpty()) {
			...
			if (pManager)
			{
				CStdPtrArray* aOptionGroup = pManager->GetOptionGroup(m_sGroupName);
				for (int i = 0; i < aOptionGroup->GetSize(); ++i) {
					CMenuElementUI* pControl = static_cast<CMenuElementUI*>(aOptionGroup->GetAt(i));
					if (pControl)
					{
						it = pCheckInfo->find(pControl->GetName());
						it->second = !(pControl != this);
					}
				}
			}
		}else
...
{
	std::map<CDuiString, bool>* pCheckInfo = CMenuWnd::GetGlobalContextMenuObserver().GetMenuCheckInfo();
	if (!m_bCheckSwitch || pCheckInfo == NULL || pCheckInfo->size() == 0)
		return false;
...
void CMenuElementUI::SetCheckSwitch(bool bCheckItem/* = false*/)
{
	m_bCheckSwitch = bCheckItem;
}

bool CMenuElementUI::GetCheckSwitch() const
{
	return m_bCheckSwitch;
}
...
void CMenuElementUI::SetAttribute(LPCTSTR pstrName, LPCTSTR pstrValue)
{
	...
	else if( _tcscmp(pstrName, _T("checkswitch")) == 0 ) {
		SetCheckSwitch(_tcscmp(pstrValue, _T("true")) == 0 ? true : false);
	}
	else if( _tcscmp(pstrName, _T("checked")) == 0 ) {
		std::map<CDuiString, bool>* pCheckInfo = CMenuWnd::GetGlobalContextMenuObserver().GetMenuCheckInfo();
		if (pCheckInfo != NULL && pCheckInfo->find(GetName()) == pCheckInfo->end())
...
LPCTSTR CMenuElementUI::GetGroup() const
{
	return m_sGroupName;
}

void CMenuElementUI::SetGroup(LPCTSTR pStrGroupName)
{
	CPaintManagerUI* pManager = CMenuWnd::GetGlobalContextMenuObserver().GetManager();
	...
	if (pStrGroupName == NULL) {
		...
		if (!m_sGroupName.IsEmpty() && pManager)
			pManager->RemoveOptionGroup(m_sGroupName, this);
		m_sGroupName = pStrGroupName;
	}

	if (!m_sGroupName.IsEmpty()) {
		if (pManager) pManager->AddOptionGroup(m_sGroupName, this);
	}
	else {
		if (pManager) pManager->RemoveOptionGroup(m_sGroupName, this);
	}

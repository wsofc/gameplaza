...
LPCTSTR CFaceSelUI::GetClass() const
{
	return _T("FaceCtrl");
}

LPVOID CFaceSelUI::GetInterface(LPCTSTR pstrName)
{
	if( _tcscmp(pstrName, _T("FaceCtrl")) == 0 ) return static_cast<CFaceSelUI*>(this);
	return CControlUI::GetInterface(pstrName);
}

void CFaceSelUI::SetLineColor(COLORREF color)
{
	m_clrLine = color;
}

void CFaceSelUI::SetFocusBorderColor(COLORREF color)
{
	m_clrFocusBorder = color;
}

void CFaceSelUI::SetZoomBorderColor(COLORREF color)
{
	m_clrZoomBorder = color;
}

void CFaceSelUI::SetRowAndCol(int nRow, int nCol)
{
	m_nRow = nRow;
	m_nCol = nCol;
}

void CFaceSelUI::SetItemSize(int nWidth, int nHeight)
{
	m_nItemWidth = nWidth;
	m_nItemHeight = nHeight;
}

void CFaceSelUI::SetZoomSize(int nWidth, int nHeight)
{
	m_nZoomWidth = nWidth;
	m_nZoomHeight = nHeight;
}

void CFaceSelUI::SetFaceList(CFaceSelList * lpFaceList)
{
	m_lpFaceList = lpFaceList;
}

void CFaceSelUI::SetCurPage(int nPageIndex)
{
...
CFaceInfo * CFaceSelUI::GetFaceInfo(int nItemIndex)
{
	...
	if( event.Type == UIEVENT_BUTTONDOWN )
	{
		...
		if (nIndex != -1)
		{
			...
			if (lpFaceInfo != NULL && m_pManager != NULL)
				m_pManager->SendNotify(this, DUI_MSGTYPE_CLICK, 0, nIndex);
		}
		return;
	}
...
void CFaceSelUI::SetAttribute(LPCTSTR pstrName, LPCTSTR pstrValue)
{
	CControlUI::SetAttribute(pstrName, pstrValue);
}

void CFaceSelUI::PaintBkColor(HDC hDC)
{
	CControlUI::PaintBkColor(hDC);
	DrawLine(hDC);
}

void CFaceSelUI::PaintBkImage(HDC hDC)
{
	CControlUI::PaintBkImage(hDC);
}

void CFaceSelUI::PaintStatusImage(HDC hDC)
{
	RECT rcItem, rcImage;
	int nRow = 0, nCol = 0;
...
void CFaceSelUI::CalcCenterRect(RECT& rcDest, int cx, int cy, RECT& rcCenter)
{
...
int CFaceSelUI::HitTest(POINT pt)
{
	RECT rcItem;
	int nRow = 0, nCol = 0;
	int nLeft = m_rcItem.left, nTop = m_rcItem.top;
...
void CFaceSelUI::CalcZoomRect(POINT point)
{
...
BOOL CFaceSelUI::GetItemRect(int nItemIndex, RECT& rect)
{
	::SetRectEmpty(&rect);
...
CGifImage * CFaceSelUI::GetZoomImage()
{
...
void CFaceSelUI::DrawLine(HDC hDC)
{
...
void CFaceSelUI::DrawFocusBorder(HDC hDC, const RECT& rect)
{
...
void CFaceSelUI::DrawZoomImage(HDC hDC)
{
...
BOOL CFaceSelUI::LoadImage(int nPageIndex)
{
	if (NULL == m_lpFaceList)
		return FALSE;
	...
	for (int i = nStart; i < nEnd; i++)
	{
		...
		if (lpFaceInfo != NULL)
		{
			...
			if (lpImage != NULL)
			{
				BOOL bRet = FALSE;
			#ifdef UNICODE
				bRet = lpImage->LoadFromFile(lpFaceInfo->m_strFileName.c_str());
			#else
				LPWSTR wstr = StringConvertor::AnsiToWide(lpFaceInfo->m_strFileName.c_str());
				bRet = lpImage->LoadFromFile(wstr);

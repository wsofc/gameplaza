...
CTempldateHelper<IModeluInterface>::CTempldateHelper(REFGUID Guid, DWORD dwVersion, LPCTSTR pszModuleDll, LPCSTR pszCreateProc) : m_dwVersion(dwVersion), m_Guid(Guid)
{
	...
	//组件属性
	lstrcpynA(m_szCreateProc,pszCreateProc,ArrayCount(m_szCreateProc));
	lstrcpyn(m_szModuleDllName,pszModuleDll,ArrayCount(m_szModuleDllName));
...
bool CTempldateHelper<IModeluInterface>::CreateInstance()
{
	...
	try
	{
		...
		{
			_sntprintf(m_szDescribe,ArrayCount(m_szDescribe),TEXT("“%s”模块加载失败"),m_szModuleDllName);
			return false;
		}
		...
		if (CreateProc==NULL) 
		{
			_sntprintf(m_szDescribe,ArrayCount(m_szDescribe),TEXT("找不到组件创建函数“%s”"),m_szCreateProc);
			return false;
		}
		...
		{
			_sntprintf(m_szDescribe,ArrayCount(m_szDescribe),TEXT("调用函数“%s”生成对象失败"),m_szCreateProc);
			return false;
		}
	}
	catch (LPCTSTR pszError)
	{
		_sntprintf(m_szDescribe,ArrayCount(m_szDescribe),TEXT("由于“%s”,组件创建失败"),pszError);
		return false;
	}
	catch (...)	
	{ 
		_sntprintf(m_szDescribe,ArrayCount(m_szDescribe),TEXT("组件创建函数“%s”产生未知异常错误,组件创建失败"),m_szCreateProc);
		return false;
	}
...
VOID CTempldateHelper<IModeluInterface>::SetModuleCreateInfo(LPCTSTR pszModuleDllName, LPCSTR pszCreateProc)
{
	//设置信息
	lstrcpynA(m_szCreateProc,pszCreateProc,ArrayCount(m_szCreateProc));
	lstrcpyn(m_szModuleDllName,pszModuleDllName,ArrayCount(m_szModuleDllName));
